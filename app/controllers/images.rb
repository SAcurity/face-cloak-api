  ...
  class Api < Roda
    route('images') do |routing|
      ...
      routing.get true do
        ...
        viewable_images = ImagePolicy::AccountScope.new(viewer, auth_scope: current_auth_scope).viewable
        images_data = viewable_images.map do |image|
          output = image.to_h
          output[:policies] = ImagePolicy.new(viewer, image, auth_scope: current_auth_scope).index_summary
          output
        end
      ...
      routing.post true do
        ...
        routing.halt 403, { message: 'Forbidden' }.to_json unless current_auth_scope.can_write?(ImagePolicy::RESOURCE)
        ...
        new_data = parse_image_upload(routing, owner_id: viewer.id)
      ...
      routing.on String do |id|
        routing.is 'raw' do
          routing.get do
            ...
            policy = ImagePolicy.new(viewer, image, auth_scope: current_auth_scope)
            routing.halt 404, { message: 'Image not found' }.to_json unless policy.can_manage_faces?
        ...
        routing.is 'face_records' do
          ...
          routing.get do
            ...
            faces_data = image.ordered_face_records.map do |fr|
              output = fr.to_h
              output[:policies] = FaceRecordPolicy.new(viewer, fr, auth_scope: current_auth_scope).index_summary
              output
            end
        ...
        routing.is do
          ...
          routing.get do
            ...
            ext = File.extname(image.file_name).delete('.')
            response['Content-Type'] = "image/#{ext}"
            response['X-Policy-Summary'] =
              ImagePolicy.new(viewer, image, auth_scope: current_auth_scope).summary.to_json
